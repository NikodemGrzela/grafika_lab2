       ...
       return fract(tan(distance(xy*PHI, xy)*seed)*xy.x);
        ...
        void main(void) {
            ...
            gl_Position = P_matrix * V_matrix * M_matrix *
                            (position + offset + (gl_InstanceID % 10) * gold_noise(position.xy,gl_VertexID)/10 * vec4(1,0,0,0));
        }
...
def startup():
    ...
    vertex_positions = numpy.array([
        -0.25, +0.25, -0.25, +0.1, +0.9, +0.1, +0.5,
        -0.25, -0.25, -0.25, +0.1, +0.9, +0.1, +0.5,
        +0.25, -0.25, -0.25, +0.1, +0.9, +0.1, +0.5,
        ...
        +0.25, -0.25, -0.25, 0.7, 0.1, 0.1, 1.0,
        +0.25, -0.25, +0.25, 0.7, 0.1, 0.1, 1.0,
        +0.25, +0.25, -0.25, 0.7, 0.1, 0.1, 1.0,
        ...
        +0.25, -0.25, +0.25, 0.7, 0.9, 0.9, 1.0,
        -0.25, -0.25, +0.25, 0.7, 0.9, 0.9, 1.0,
        +0.25, +0.25, +0.25, 0.7, 0.9, 0.9, 1.0,
        ...
        -0.25, -0.25, +0.25, 0.7, 0.4, 0.9, 1.0,
        -0.25, -0.25, -0.25, 0.7, 0.4, 0.9, 1.0,
        -0.25, +0.25, +0.25, 0.7, 0.4, 0.9, 1.0,
        ...
        -0.25, -0.25, +0.25, 0.3, 0.6, 0.9, 1.0,
        +0.25, -0.25, +0.25, 0.3, 0.6, 0.9, 1.0,
        +0.25, -0.25, -0.25, 0.3, 0.6, 0.9, 1.0,
        ...
        -0.25, +0.25, -0.25, 0.1, 0.9, 0.9, 1.0,
        +0.25, +0.25, -0.25, 0.1, 0.9, 0.9, 1.0,
        +0.25, +0.25, +0.25, 0.1, 0.9, 0.9, 1.0,
    ...
    for y in range(10):
        for x in range(10):
            translation = glm.vec4(
                float(x) % 10.0,
                float(y) % 10.0,
                0.0,
                0.0
            )
            translations_list.extend([translation.x, translation.y, translation.z, translation.w])

    translations = numpy.array(translations_list, dtype='float32')
    ...
    translation_buffer = glGenBuffers(1)
    glBindBuffer(GL_ARRAY_BUFFER, translation_buffer)
    glBufferData(GL_ARRAY_BUFFER, translations, GL_STATIC_DRAW)

    glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), ctypes.c_void_p(0))
